...
class piece:
    def __init__(self):
        ...
        self.Values = {self.WhP:-1, self.WhR:-5, self.WhKn:-3, self.WhB:-3, self.WhQ:-9, self.WhK:-1000, self.BlP:1, self.BlR:5, self.BlKn:3, self.BlB:3, self.BlQ:9, self.BlK:1000}
...
class game:
    ...
    def playMove(self, move):
        ...
        if special == "" or special == "WK" or special == "BK" or (len(special) >= 3 and special[0:3] == "PPr"):
            if start == (1, 1) and self.board[start] == Piece.WhR:
                self.currentConditions.hasWRookQMoved = True
            elif start == (8, 1) and self.board[start] == Piece.WhR:
                self.currentConditions.hasWRookKMoved = True
            elif start == (1, 8) and self.board[start] == Piece.BlR:
                self.currentConditions.hasBRookQMoved = True
            elif start == (8, 8) and self.board[start] == Piece.BlR:
                self.currentConditions.hasBRookKMoved = True
             
            if end in self.board:
                self.previousTaken.append(self.board[end])

                if end == (1, 1) and self.board[end] == Piece.WhR:
                    self.currentConditions.hasWRookQMoved = True
                elif end == (8, 1) and self.board[end] == Piece.WhR:
                    self.currentConditions.hasWRookKMoved = True
                elif end == (1, 8) and self.board[end] == Piece.BlR:
                    self.currentConditions.hasBRookQMoved = True
                elif end == (8, 8) and self.board[end] == Piece.BlR:
                    self.currentConditions.hasBRookKMoved = True
    ...
    def getPointsSum(self):
        return sum([Piece.Values[i] if i is not None else 0 for i in self.previousTaken])
...
def naiveStaticEvalAgent(game, model):
    ...
    if game.currentConditions.whiteTurn:
        ...
        for key, value in game.availableMoves.items():
            for move in value:
                # print("white turn to move: ", move)
                game.playMove(move)
                game_result = game.isGameOver()[1]
                ...
                prediction = model.predict(tuple_to_input(game.hashBitBoard()), verbose=0)[0][0]
                print("prediction: ", prediction)
                evals.append(prediction)
                moves.append(move)
                game.reverseMove()
        # print(len(evals), len(moves))
        # print(moves)
        min_eval = min(evals)
        if min_eval < 0:
            evals = [e + abs(min_eval) for e in evals]

        indices = np.arange(len(moves))
        evals = np.array(evals)
        probabilities_white = np.exp(evals) / np.sum(np.exp(evals))

        ind = np.random.choice(indices, p=probabilities_white)
        game.playMove(moves[ind])
        ...
        for key, value in game.availableMoves.items():
            for move in value:
                # print("black turn to move: ", move)
                game.playMove(move)
                game_result = game.isGameOver()[1]
                if game_result == "black":
                    return
                elif game_result == "white":
                    evals.append(100)
                    moves.append(move)
                    continue
        ...
        indices = np.arange(len(moves))
        evals = np.array(evals)
        probabilities_black = np.exp(-evals) / np.sum(np.exp(-evals))

        # Choose a move based on the probability distribution for black
        ind = np.random.choice(indices, p=probabilities_black)
        game.playMove(moves[ind])
